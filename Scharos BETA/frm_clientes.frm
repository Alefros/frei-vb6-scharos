...
Private Sub cmd_excluir_Click()
            ...
            tabcli.Close
            tabcli.Open "select * from Clientes where Codigo like '" & txt_codcli & "'"
            If MsgBox("Deseja realmente excluir este cliente?", vbYesNo + vbDefaultButton2 + vbQuestion, "Scharos BETA") = vbYes Then
            If tabcli.RecordCount <> 0 Then
...
Private Sub cmd_gravar_Click()
        ...
        If opt_fisica.Value = True Then 'opção se pessoa fisica estiver selecionada
            ...
            tabcli.Open "select * from clientes where Rg like '" & msk_rg & "'"
                If tabcli.RecordCount = 1 Then
                    MsgBox "Este Rg já está cadastrado, favor verificar", vbInformation, "Scharos BETA"
                    msk_rg = Empty
                    msk_rg.SetFocus
                    Exit Sub
                ElseIf tabcli.RecordCount = 0 Then
                        ...
                        tabcli.Open "select * from clientes where Cpf like '" & msk_cpf & "'"
                            If tabcli.RecordCount = 1 Then
                                MsgBox "Este Cpf já está cadastrado, favor verificar", vbInformation, "Scharos BETA"
                                msk_cpf = Empty
                                msk_cpf.SetFocus
                                Exit Sub
        End If
        ...
        If opt_juridica.Value = True Then 'opção se pessoa juridica estiver selecionada
            ...
            tabcli.Open "select * from clientes where Cnpj = '" & msk_cnpj & "'"
                If tabcli.RecordCount = 1 Then
                    If tabcli!cnpj = "" Then
                        MsgBox "Este Cnpj já está cadastrado, favor verificar", vbInformation, "Scharos BETA"
                        msk_cnpj = Empty
                        msk_cnpj.SetFocus
                        Exit Sub
                    End If
...
Private Sub cmd_validar_Click()
           msk_cpf.PromptInclude = False
            ...
            If StrConf <> Right(CPF, 2) Then
               MsgBox "Número do CPF Inválido. Por favor, tente novamente.", vbInformation, "Scharos BETA"
                       msk_cpf = Empty
...
Private Sub opt_fisica_Click()
                opt_juridica.Value = False
            If opt_fisica.Value Then
            Call cor
                 With txt_rsocial
                    .Enabled = False
                    '.BackColor = &HE0E0E0
...
End Sub
Private Sub cor() ' Esta função altera a possibilidade de preenchimento de alguns valor no formulário, considerando pessoa física ou jurídica
            If opt_fisica.Value = True Then
                
                With txt_rsocial
                    .Enabled = False
                    .BackColor = &HE0E0E0       ' Desabilita e deixa com uma cor mais próxima ao cinza do formulário
                 End With                                                   '  as caixas referente à pessoa jurídica
                 With msk_cnpj
                    .Enabled = False
                    .BackColor = &HE0E0E0
                 End With
...
End Sub
Private Sub fechar() ' fechar tabelas do BD Scharos
...
End Sub
Private Sub abrir() ' abrir tabelas do BD Scharos
            Call fechar
...
Private Sub logra()
            ...
            ElseIf tab_loca.RecordCount = 0 Then
                    MsgBox "Código de Endereçamento Postal (CEP) não encontrado, favor verificar", vbExclamation, "Scharos BETA"
                        msk_cep = Empty
                        codbairro = Empty
                        codcidade = Empty
                        coduf = Empty
...
Private Sub clcc()
                Call abrir
                tabcli.Close
                tabcli.Open "Select * from Clientes where " & criterio & " = '" & valor & "'"
                If tabcli.RecordCount = 0 Then
                    MsgBox "Não Existem clientes que possuem este(a) " & cbo_criterio & "", vbInformation, "Scharos BETA"
